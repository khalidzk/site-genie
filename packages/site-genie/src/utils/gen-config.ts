...
export const genConfigByName = async (template: string, configName = 'siteGenie.config.ts') => {
  const plugins = template.split('_').filter((p) => p !== '');
  let option: any = {};
  for (let i = 0; i < plugins.length; i++) {
    const item = plugins[i];
    const pluginName = `@site-genie/plugin-${item}`;
...
const genConfig = ({ from, to, plugin }: any) => {
  return `import { defineConfig } from '@site-genie/site-genie'
