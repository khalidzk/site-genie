...
export class PluginDriver {
  ...
  constructor(option: SiteGenieConfig) {
    this.plugins = this.normalizeOptions(option);
    this.pluginContexts = new Map(this.plugins.map((plugin) => [plugin, getPluginContext()]));
  }

  normalizeOptions(option: SiteGenieConfig): IPlugin[] {
    if (!option.from) {
      throw new Error('You must supply an `from` option to siteGenie');
    }
    if (!option.to) {
      throw new Error('You must supply an `to` option to siteGenie');
    }
